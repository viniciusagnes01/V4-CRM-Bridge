...
async function requestMoskit(url, accessKey) {
  ...
  return { ok: true, status: response.status, bodyType: Array.isArray(json) ? 'array' : typeof json, keys: json && typeof json === 'object' ? Object.keys(json).slice(0, 20) : [], list: listFromResponse(json) };
}

export default async function handler(req, res) {
  try {
    const accessKey = process.env.MOSKIT_ACCESS_KEY;
    if (!accessKey) return res.status(500).json({ ok: false, message: 'MOSKIT_ACCESS_KEY missing' });
